(ns satcom.operation
  "OperationActor -- one satellite-operator operation = one supervised
  actor run, expressed as a langgraph-clj StateGraph. The advisor
  (Satellite Operations Advisor) is sealed into a single node
  (:advise); its proposal is ALWAYS routed through the Satellite
  Network Governor (:govern) and the rollout phase gate (:decide)
  ...
  One graph run = one satellite-operator operation (intake -> advise ->
  ...
  decision to a human operator (the satellite operator). The approver
  resumes with `{:approval {:status :approved}}` (or :rejected).
  `:actuation/provision-capacity`/`:actuation/suspend-service` ALWAYS
  reach this node when the governor is clean -- see `satcom.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [satcom.satcomadvisor :as satcomadvisor]
            [satcom.governor :as governor]
...
(defn build
  ...
  `satcom.store/Store`).
  opts:
    :advisor      -- a `satcom.satcomadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (satcomadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Satellite Operations Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (satcomadvisor/-advise advisor store request)]
            {:proposal p :audit [(satcomadvisor/trace request p)]})))

      ;; Satellite Network Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
